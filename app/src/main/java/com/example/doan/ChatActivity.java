...
public class ChatActivity extends AppCompatActivity {
    ...
    private void insertUser(){
        ...
        user.put("customerid",Utils.user_current.getCustomerId());
        user.put("nameC",Utils.user_current.getNameC());
        db.collection("Users").document(String.valueOf(Utils.user_current.getCustomerId())).set(user);
    }
    /////////////////////////////////////
    ...
    private void sendMessToFire() {
        ...
        if (TextUtils.isEmpty(str_mess)) {
            ...
            db.collection(Utils.PATH_CHAT).add(message).addOnSuccessListener(documentReference -> {
                ...
                chatMessage.received = Utils.ID_RECEIVED;
    ...
    if (value != null) {
        int count = list.size(); // Lưu lại số lượng hiện tại trước khi cập nhật
        ...
        if (count == 0) {
            ...
            adapter.notifyItemRangeChanged(count, list.size() - count);
            recyclerView.smoothScrollToPosition(list.size() - 1);
        }
    }
};
